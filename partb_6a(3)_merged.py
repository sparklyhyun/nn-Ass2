...
def main():
    ...
    logits, word_list = rnn_model_rnn(x)
    ...
    entropy2 = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.one_hot(y2_, MAX_LABEL), logits=logits2))
    ...
    entropy3 = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits_v2(labels=tf.one_hot(y3_, MAX_LABEL), logits=logits3))
    ...
    with tf.Session() as sess:
        ...
        for e in range(no_epochs):
            ...
            for start, end in zip(range(0, N, batch_size), range(batch_size, N, batch_size)):
                ...
                #lstm
                word_list2_, _, loss2_ = sess.run([word_list2, train_op2, entropy2],
                                                {x2: trainX_batch[start:end], y2_: trainY_batch[start:end]})
                loss_batch2.append(loss2_)

                # gru
                word_list3_, _, loss3_ = sess.run([word_list3, train_op3, entropy3],
                                                  {x3: trainX_batch[start:end], y3_: trainY_batch[start:end]})
        ...
        pylab.savefig('figures/partb_6a(3)_entropy_merged.png')
        ...
        pylab.legend(['Basic RNN', 'LSTM', 'GRU'])
        pylab.savefig('figures/partb_6a(3)_accuracy_merged.png')
