...
def rnn_model2(x):
    ...
    cells2 = tf.nn.rnn_cell.MultiRNNCell([cell1, cell2])
    outputs, states = tf.nn.static_rnn(cells2, word_list2, dtype=tf.float32)

    logits = tf.layers.dense(states[-1], MAX_LABEL, activation=tf.nn.softmax)

    return logits, word_list2
...
def main():
    ...
    logits2, word_list2 = rnn_model2(x)
    print('here2')
